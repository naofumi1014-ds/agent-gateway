...
st.set_page_config(
    page_title="Snowflake Cortex Agents Demo App",
    page_icon="❄️",
    layout="wide",
...
if "agent" not in st.session_state:
    st.session_state.agent_gateway = AgentGateway()
    st.session_state.agent_gateway.enable_cross_region_inference()
    st.session_state.snowflake_tools = st.session_state.agent_gateway.initialize_tools()
    st.session_state.agent = Agent(snowflake_connection=st.session_state.agent_gateway.connection, 
...
st.markdown("### ❄️Snowflake Cortex Agents Demo App")

with st.expander("アプリの説明", expanded=True):
    st.write("Snowflake Cortex Agents 機能を使用したデモアプリです。")
    st.write("Snowflake内にあるデータに対し、Cortex Agentsというエージェントサービスを利用しユーザーの質問に対して回答を行います。")
    st.write("このデモでは、Conrtex Analyst,SearchサービスをToolとして使用しています。")
